...
client.on('message', msg => {
  if (msg.content.toLowerCase() === 'd!davet') {
    msg.reply('Botun Sınırsız Davet Linki : https://discord.com/oauth2/authorize?client_id=790354206040653844&scope=bot&permissions=8');
  }
});
...
if (message.content === 'd!fakekatıl') { // . yerine prefixi yaz
...
// eklendim
client.on('guildCreate', async guild => { client.channels.get('791042486831284224').send(`${guild}, isimli sunucuya eklendim!`)})
// atıldım
client.on('guildRemove', async guild => { client.channels.get('791042486831284224').send(`${guild}, isimli sunucudan atıldım.. :(`)})
...
client.on('message', async message => {
  ...
  if (message.guild.channels.find(channel => channel.name === "bot-komut")) return message.channel.send(" Bot Paneli Zaten Ayarlanmış.")
...
//
client.on("message", msg => {
  var dm = client.channels.get("791042486831284224"); //mesajın geleceği kanal idsi//
